...
% divided by function
Application software, on the other hand, would be divided into parts, each containing single function. Each application function would be combined with a short header (table \ref{tab:header}) to form a package. Each function could then be independently updated, added or removed.
...
Main focus is on storing application functions in internal Flash, since it is the most restrictive storage. Additionally, in safe mode, this is the sole non-volatile storage available. Storage of some functions in other memories, as well as moving functions between different memories and addresses, would also be possible.
...
Compiler will have a header available that contains preprocessor function-like macros matching each application function (figure \ref{fig:macro}). They will contain instructions to read function address from function table, cast it to the function's type, cast all arguments to required types, and call the function.
...
\end{lstlisting}
\caption{Call interception macro for hypothetical function \texttt{f0()}.}
\label{fig:macro}
\end{figure}

The macro that corresponds to the function being compiled will be removed using \texttt{\#undefine}. This way application code can be written identically regardless if updating system will be used or not.
...
In order to simplify the design, globals would be treated very similarly to functions. First, they would be compiled, so that compiler would calculate their lengths. Compilation should be \texttt{-fdata-sections}, then they can be extracted to separate packages, prepended with headers, and uploaded to the satellite.
